...
public class JSBMLWriter1 implements TreeModelListener{
	...
	public void formConnect(LocalConfig config) throws Exception{
		...
		System.out.println(curConfig.getMaxMetaboliteId());
	...
	public class SMetabolites {
		...
		public void parseAllMetabolites() {
			...
			int length = curConfig.getMaxMetaboliteId();
			...
			System.out.print("\n");
			Vector<ModelMetabolite> curMetas = mFactory.getAllInternalMetabolites();
			
			System.out.println(mFactory.getMetaboliteById(1, sourceType, databaseName).toString())		;//.get(1));
			for (int i=1; i <= length; i++) {
				SBMLMetabolite curMeta = (SBMLMetabolite) curMetas.get(i);//mFactory.getMetaboliteById(i, sourceType, databaseName);
